...
import { tasksTypes, tasksActions } from "../store/tasksTypesActions";
...
const Todo = ({ label, done, onRemove, id, onToggle, onSelect }) => {
  ...
  return (
    <div style={{ display: "flex", justifyContent: "space-between" }}>
      ...
      <div>
        <span>
          <button className="btn btn-success" onClick={() => onToggle(id)}>
            {done ? "Не готово" : "Готово"}
          </button>
          <button className="btn btn-warning" onClick={() => onSelect(id)}>
            Редактировать
          </button>
          <button className="btn btn-secondary" onClick={() => onRemove(id)}>
            X
          </button>
        </span>
      </div>
...
class TodoList extends Component {
        ...
        <div>
          ...
          <section className="filters">
            ...
            <select
              ...
              ref={select => (this.filterInput = select)}
  ...
  render() {
    return (
      <section>
        {this.renderTodos()}
        <Link to="/list/add" className="btn btn-dark">
          Добавить задачу
        </Link>
      </section>
    );
